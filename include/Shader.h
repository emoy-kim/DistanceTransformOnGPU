...
public:
   ...
   [[nodiscard]] GLuint getComputeShaderProgram(int index) const { return ComputeShaderPrograms[index]; }
   ...
   void uniform1i(const char* name, int value, int index) const
   {
      glProgramUniform1i( ComputeShaderPrograms[index], CustomLocations.find( name )->second, value );
   }
